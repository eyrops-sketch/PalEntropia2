Window.PALARENA_STANDAR = (function() {
    ...
    function regenerarFatiga(combatiente) {
        ...
        combatiente.fatiga = Math.min(
            ...
            combatiente.fatiga + (Number(config.fatiga_regeneracion_turno) || 0)
        );
    }
    ...
    function ejecutarAccion(atacante, objetivo, codigoAccion) {
        ...
        const fatigaCritica = (atacante.fatiga / (Number(config.fatiga_max) || 100)) * 100 < 15;
        ...
        if (codigoAccion === "A001") costeFatiga = Number(config.coste_fatiga_A001) || 0;
        else if (codigoAccion === "A002") costeFatiga = Number(config.coste_fatiga_A002) || 0;
        else if (codigoAccion === "A003") costeFatiga = Number(config.coste_fatiga_A003) || 0;
            ...
            if (atacante.usosDefensaConsecutivos >= 3) {
                ...
                costeFatiga = -(Number(config.coste_fatiga_D001) || 15);
            }
        }
        ...
        atacante.fatiga = Math.min(Number(config.fatiga_max) || 100, Math.max(0, atacante.fatiga - costeFatiga));
        ...
        if (codigoAccion === "A001") {
            ...
            if ((objetivo.fatiga / (Number(config.fatiga_max) || 100)) * 100 < 15) {
                bonusAccion *= 1.25;
            ...
            if (comprobarCritico(atacante, config)) {
                const multiplicadorCritico = Number(config.multiplicador_critico) || 2.50;
        ...
        if (atacante.estadoCaotico && atacante.estadoCaotico > 0) {
            ...
            if (Math.random() < 0.80) {
                ...
                let recuperacionFatiga = Math.round((Number(config.fatiga_max) || 100) * 0.08);
