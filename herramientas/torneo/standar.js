Window.PALARENA_STANDAR = (function() {
    ...
    function calcularStatsEfectivos(ficha, config) {
        const valorGeneral = Number(config.general);
        const coefGeneral = Number.isFinite(valorGeneral) ? valorGeneral : 1;
        ...
        if (window.PALARENA_STATS && typeof window.PALARENA_STATS.calcularFicha === "function") {
            ...
            return {
                ataque: Number(statsCalculados.ataque) * coefGeneral,
                defensa: Number(statsCalculados.defensa) * coefGeneral,
                velocidad: Number(statsCalculados.velocidad) * coefGeneral,
                resistencia: Number(statsCalculados.resistencia) * coefGeneral,
                tactica: Number(statsCalculados.tactica) * coefGeneral
            };
        }
    ...
    function crearCombatiente(ficha, configPersonalizada = null) {
        ...
        const hpMax = Number(config.hp_base) * (efectivos.resistencia / 50);
        ...
        return {
            ...
            hp_max: hpMax,
            hp: hpMax,
            fatiga_max: Number(config.fatiga_max) || 100,
            fatiga: Number(config.fatiga_inicial) || 100,
            efectivos: efectivos,
            defendiendo: false,
            derrotado: false,
    ...
    function regenerarFatiga(combatiente) {
        const config = configuracionGlobal;
        const regeneracion = Number(config.fatiga_regeneracion_turno) || 0;
        combatiente.fatiga = Math.min(
            combatiente.fatiga_max,
            combatiente.fatiga + regeneracion
        );
    }

    function obtenerPotenciaAccion(codigoAccion) {
        if (window.PALARENA_STANDAR_ATAQUES && typeof window.PALARENA_STANDAR_ATAQUES.obtener === "function") {
            const ataque = window.PALARENA_STANDAR_ATAQUES.obtener(codigoAccion);
            if (ataque && Number.isFinite(Number(ataque.potencia))) {
                return Number(ataque.potencia);
            }
        }
        if (codigoAccion === "A001") return 1.00;
        if (codigoAccion === "A002") return 1.40;
        if (codigoAccion === "A003") return 0.80;
        return 1.00;
    }

    function calcularProbabilidadCritico(atacante, config) {
        const base = Number(config.critico_base);
        const coefTactica = Number(config.critico_tactica);
        const tactica = Number(atacante.efectivos.tactica) + Number(atacante.tacticaTemporal || 0);
        const baseCritico = Number.isFinite(base) ? base : 0;
        const factorTactica = Number.isFinite(coefTactica) ? coefTactica : 0;
        const probabilidad = baseCritico + (tactica * factorTactica);
        return Math.max(0, Math.min(35, probabilidad));
    }

    function comprobarCritico(atacante, config) {
        const fatigaMinima = Number(config.fatiga_minima_critico);
        const minimoFatiga = Number.isFinite(fatigaMinima) ? fatigaMinima : 0;
        if (Number(atacante.fatiga) < minimoFatiga) {
            return false;
        }
        const probabilidad = calcularProbabilidadCritico(atacante, config);
    ...
    function ejecutarAccion(atacante, objetivo, codigoAccion) {
        ...
        const fatigaCritica = (Number(atacante.fatiga) / (Number(config.fatiga_max) || 100)) * 100 < 15;
        ...
        if (codigoAccion === "A001") {
            costeFatiga = Number(config.coste_fatiga_A001) || 0;
        } else if (codigoAccion === "A002") {
            costeFatiga = Number(config.coste_fatiga_A002) || 0;
        } else if (codigoAccion === "A003") {
            costeFatiga = Number(config.coste_fatiga_A003) || 0;
        } else if (codigoAccion === "D001") {
            if (atacante.usosDefensaConsecutivos === undefined) {
                atacante.usosDefensaConsecutivos = 0;
            }
            atacante.usosDefensaConsecutivos++;
            if (atacante.usosDefensaConsecutivos >= 3) {
                costeFatiga = Number(config.coste_fatiga_D001) || 20;
            } else {
                costeFatiga = -(Number(config.coste_fatiga_D001) || 20);
            }
        }
        ...
        atacante.fatiga = Math.min(
            Number(config.fatiga_max) || 100,
            Math.max(0, atacante.fatiga - costeFatiga)
        );
        ...
        if (codigoAccion === "D001") {
            if (objetivo.defendiendo) {
                ...
                if (puntuacionAtacante > puntuacionObjetivo) {
                    ...
                    objetivo.fatiga = Math.max(0, objetivo.fatiga - penalizacion);
                    ...
                    atacante.fatiga = Math.max(0, atacante.fatiga - penalizacionPropia);
            ...
            const umbralError = Math.max(
                ...
                (Number(config.error_defensa_base) || 45) -
                (atacante.efectivos.tactica * (Number(config.error_defensa_reduccion_tactica) || 0.02))
            );
        ...
        if (codigoAccion === "A001") {
            ...
            const bonusCadena = 1.0 + (Math.min(atacante.rachaBasicos, 4) - 1) * 0.12;
            bonusAccion *= obtenerPotenciaAccion("A001") * bonusCadena;

            if (atacante.rachaBasicos > 1) {
                mensajeExtra = ` (Cadena de básicos x${atacante.rachaBasicos}, presión ofensiva desatada)`;
            }

            if ((atacante.efectivos && Number(objetivo.fatiga) / (Number(config.fatiga_max) || 100)) * 100 < 15) {
                bonusAccion *= 1.25;
            ...
            bonusAccion *= obtenerPotenciaAccion("A002");
            if (comprobarCritico(atacante, config)) {
                const multiplicadorCritico = Number(config.multiplicador_critico) || 1.0;
                bonusAccion *= multiplicadorCritico;
                critico = true;
                mensajeExtra = ` 💥 ¡Golpe crítico!`;
            }
        } else if (codigoAccion === "A003") {
            const tacticaAtacante = atacante.efectivos.tactica + (atacante.tacticaTemporal || 0);
            const tacticaObjetivo = objetivo.efectivos.tactica + (objetivo.tacticaTemporal || 0);
            bonusAccion *= obtenerPotenciaAccion("A003");
        ...
        if (objetivo.defendiendo) {
            if (codigoAccion === "A002") {
                ...
                defensaObjetivo = Number(objetivo.efectivos.defensa) * (1 + (Number(config.reduccion_defensa) || 0));
            }
        } else {
        ...
        const variacion = Math.max(0, Number(config.variacion_dano) || 0.25);
        ...
        if (atacante.estadoCaotico && atacante.estadoCaotico > 0) {
            ...
            if (Math.random() < 0.80) {
                ...
                const penalizacionFatiga = Math.round((Number(config.fatiga_max) || 100) * 0.08);
                atacante.fatiga = Math.max(0, atacante.fatiga - penalizacionFatiga);

                if (accionAleatoria !== "A001") {
                    atacante.rachaBasicos = 0;
                }
        ...
        const pesoAleatorioValor = Number(config.ia_peso_aleatorio);
        const pesoAleatorio = Number.isFinite(pesoAleatorioValor) ? pesoAleatorioValor : 0.40;
        ...
        if (Math.random() < pesoAleatorio) {
            ...
            if (fatigaActual >= fatigaMinPotente && fatigaActual >= costePotente) {
    ...
    function obtenerCombatiente(combate, codigo) {
        if (combate.combatiente1.codigo === codigo) {
            return combate.combatiente1;
        }
        if (combate.combatiente2.codigo === codigo) {
    ...
    function reiniciarSerieCombatientes(combate) {
        ...
        if (combate.combatiente1) {
            ...
            combate.combatiente1.fatiga = Number(configuracionGlobal.fatiga_inicial) || 100;
        ...
        if (combate.combatiente2) {
            ...
            combate.combatiente2.fatiga = Number(configuracionGlobal.fatiga_inicial) || 100;
...
window.ejecutarTurnoEstandar = function(combate) {
    if (!combate || combate.estado === "finalizado") {
        return;
    }
    ...
    if (c1.tacticaTemporal !== 0) {
        c1.tacticaTemporal = Math.sign(c1.tacticaTemporal) * Math.max(0, Math.abs(c1.tacticaTemporal) - 2);
    }
    if (c2.tacticaTemporal !== 0) {
        c2.tacticaTemporal = Math.sign(c2.tacticaTemporal) * Math.max(0, Math.abs(c2.tacticaTemporal) - 2);
    }
