...
window.PALARENA_STANDAR = {
    ...
    limitarCoeficiente(valor) {
        const min = this.configuracion.coeficiente_combate_min;
        const max = this.configuracion.coeficiente_combate_max;
        return Math.max(min, Math.min(max, this.numero(valor)));
    },
    aleatorio(min, max) {
    ...
    obtenerCoeficienteCombate(ficha, reglas) {
        let coef = this.configuracion.coeficiente_combate;
        if (ficha && Number.isFinite(Number(ficha.coeficiente_combate))) {
            coef = Number(ficha.coeficiente_combate);
        }
        if (reglas && reglas.coeficientes_combate && ficha) {
            ...
            if (Number.isFinite(Number(reglas.coeficientes_combate[codigo]))) {
                coef = Number(reglas.coeficientes_combate[codigo]);
            }
        }
        if (reglas && Number.isFinite(Number(reglas.coeficiente_combate))) {
            coef = Number(reglas.coeficiente_combate);
        }
        return this.limitarCoeficiente(coef);
    },
    ...
    obtenerDominante(indicadores) {
        ...
        const maximo = Math.max(...atributos.map(item => item.valor));
        let candidatos = atributos.filter(item => item.valor === maximo);
        if (candidatos.length > 1) {
            const tactica = candidatos.find(item => item.atributo === "tactica");
    ...
    crearCombatiente(ficha, reglas) {
        ...
        const hpMax = this.configuracion.hp_base + Math.round(efectivos.resistencia);
        return {
    ...
    aplicarFatigaAccion(atacante, codigo) {
        const coste = this.obtenerCosteFatiga(codigo);
    ...
    obtenerEfecto(combatiente, codigo) {
        ...
        return combatiente.efectos.find(efecto => efecto.codigo === codigo) || null;
    },
    ...
    procesarEfectos(combatiente) {
        ...
        const resultados = [];
        combatiente.efectos.forEach(efecto => {
        ...
        combatiente.efectos = combatiente.efectos.filter(efecto => efecto.turnos > 0);
        if (combatiente.hp <= 0) {
            combatiente.hp = 0;
    ...
    comprobarEsquiva(objetivo) {
        ...
        return { esquiva: dado < probabilidad, probabilidad, dado };
    },
    comprobarFallo(atacante) {
        ...
        const dado = this.aleatorio(0, 100);
        return { fallo: dado < probabilidad, probabilidad, dado };
    },
    comprobarDefensa(defensor) {
        ...
        return { error, acierto: !error, probabilidad_error: probabilidad, dado };
    },
    comprobarCritico(atacante, ataque) {
        ...
        const dado = this.aleatorio(0, 100);
        return { critico: dado < probabilidad, probabilidad, dado, bloqueado_por_fatiga: false };
    },
    ...
    obtenerEfectoPerfil(combatiente) {
       ...
       decidirAccion(atacante, objetivo) {
        ...
        puntuaciones.A001 += ataque * 0.20 + resistencia * 0.10;
        if (hpObjetivo <= 50) { puntuaciones.A001 += 8; }
        if (fatigaObjetivo <= 30) { puntuaciones.A001 += 5; }
        puntuaciones.A002 += ataque * 0.35 + tactica * 0.15;
        ...
        puntuaciones.A003 += tactica * 0.30 + inteligencia * 0.15;
        if (fatigaObjetivo < 50) { puntuaciones.A003 += 10; }
        if (hpObjetivo <= 50) { puntuaciones.A003 += 8; }
        puntuaciones.D001 += defensa * 0.20 + resistencia * 0.15;
        if (hpPorcentaje <= 50) { puntuaciones.D001 += 20; }
        if (hpPorcentaje <= 25) { puntuaciones.D001 += 20; }
        if (fatiga < 40) { puntuaciones.D001 += 15; }
        if (inteligencia >= 70) { puntuaciones.A002 += 8; puntuaciones.A003 += 8; puntuaciones.D001 += 5; }
        if (inteligencia >= 85) { puntuaciones.A003 += 10; }
        if (fatiga >= 75) { puntuaciones.A002 += 8; }
        if (fatiga <= 25) { puntuaciones.A001 += 10; puntuaciones.A003 += 8; puntuaciones.D001 += 8; }
        const disponibles = ["A001", "A002", "A003", "D001"];
        if (fatiga < this.configuracion.fatiga_minima_ataque_potente) {
            const idx = disponibles.indexOf("A002");
            if (idx !== -1) { disponibles.splice(idx, 1); }
        }
        const candidatos = disponibles.map(codigo => ({
            codigo,
            puntuacion: puntuaciones[codigo] + (Math.random() * 10)
        ...
        if (elegido && elegido.codigo === "A002" && fatiga < this.configuracion.fatiga_minima_ataque_potente) {
            elegido = candidatos.find(item => item.codigo !== "A002");
        }
    ...
    ejecutarAtaque(atacante, objetivo, codigo) {
        if (!atacante || !objetivo || atacante.derrotado) { return null; }
        const ataque = {
            codigo: codigo,
            nombre: codigo === "A001" ? "Ataque básico" : codigo === "A002" ? "Ataque potente" : codigo === "A003" ? "Ataque táctico" : "Defender",
            critico: codigo !== "D001",
            efecto: codigo === "A003" ? this.obtenerEfectoPerfil(atacante) : null
        };
        if (codigo === "D001") {
        ...
        if (fallo.fallo) {
            return {
                ...
                fallo: true,
                mensaje: "❌ " + atacante.nombre + " falla su ataque.",
                fatiga: fatigaAccion
            };
        }
        ...
        if (esquiva.esquiva) {
            return {
                ...
                nombre: ataque.nombre,
                esquiva: true,
                mensaje: "💨 " + objetivo.nombre + " esquiva el ataque.",
                fatiga: fatigaAccion
            };
        }
        const critico = this.comprobarCritico(atacante, ataque);
        let multiplicador = this.obtenerMultiplicadorVida(atacante);
        if (critico.critico) { multiplicador *= this.configuracion.multiplicador_critico; }
        let danoBase = this.configuracion.dano_base + (atacante.efectivos.ataque * this.configuracion.dano_por_ataque);
        let defensaObj = this.obtenerDefensa(objetivo);
        if (objetivo.defendiendo) { defensaObj *= (1 + this.configuracion.reduccion_defensa); }
        let danoFinal = Math.round((danoBase * multiplicador) - (defensaObj * 0.2));
        danoFinal = Math.max(5, danoFinal);
        ...
        return {
            ...
            mensaje: "💥 " + atacante.nombre + " causa " + danoFinal + " de daño a " + objetivo.nombre + "."
        };
    },
    finalizarCombate(combate) {
        ...
        if (c1.derrotado && c2.derrotado) {
            combate.estado = "finalizado"; combate.ganador = null; combate.perdedor = null;
        } else if (c1.derrotado) {
            combate.estado = "finalizado"; combate.ganador = c2.codigo; combate.perdedor = c1.codigo;
        } else if (c2.derrotado) {
            combate.estado = "finalizado"; combate.ganador = c1.codigo; combate.perdedor = c2.codigo;
        }
        return combate;
    },
    ejecutarTurno(combate) {
        ...
        const reg1 = this.regenerarFatiga(primero);
        const reg2 = this.regenerarFatiga(segundo);
        combate.historial.push({ tipo: "regeneracion_fatiga", turno: combate.turno, primero: reg1, segundo: reg2 });
    ...
    obtenerResumen(combate) {
        ...
        const resumenC = (c) => ({
            ...
            hp: c.hp,
            hp_max: c.hp_max,
            hp_porcentaje: (c.hp / c.hp_max) * 100,
            fatiga: c.fatiga,
            fatiga_max: c.fatiga_max,
            fatiga_porcentaje: (c.fatiga / c.fatiga_max) * 100,
            dominante: c.dominante,
            perfil: c.perfil,
            derrotado: c.derrotado,
            defendiendo: c.defendiendo,
            efectos: c.efectos
        });
        return {
            ...
            combatiente1: resumenC(combate.combatiente1),
            combatiente2: resumenC(combate.combatiente2),
...
window.ejecutarCombateEstandar = (combate) => window.PALARENA_STANDAR.ejecutarCombate(combate);
window.obtenerResumenEstandar = (combate) => window.PALARENA_STANDAR.obtenerResumen(combate);
/* ==================================================
   FIN STANDAR.JS v1.2
================================================== */
