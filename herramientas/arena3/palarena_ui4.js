...
/* ==========================================================
   FORMATEAR MEDIO ECOLÓGICO
   ========================================================== */

function generarMedioArena(medio){
    ...
    return partes.map(function(parte){
        ...
        const datos=
            window.PALARENA_DATOS.obtenerMedio(codigo);
        ...
        return `
            <div class="medio">
                <strong>${escaparHTML(datos.nombre||codigo)}</strong>
            </div>
        `;
...
/* ==========================================================
   FORMATEAR MEDIOS DEL ESCENARIO
   ========================================================== */

function generarMediosEscenarioArena(medios){

    if(!Array.isArray(medios)||!medios.length){
        return "—";
    }

    return medios.map(function(medio){
        ...
        const datosMedio=
            window.PALARENA_DATOS.obtenerMedio(medio.SM);

        const nombreSM=
            datosMedio&&datosMedio.nombre
                ?datosMedio.nombre
                :"";

        const localizacion=
            window.PALARENA_DATOS.obtenerMedio(medio.L);

        const estrato=
            window.PALARENA_DATOS.obtenerMedio(medio.ES);

        const comportamiento=
            window.PALARENA_DATOS.obtenerMedio(medio.C);

        return `
            <div class="medioEscenario">
                ${nombreSM
                    ?`<strong>${escaparHTML(nombreSM)}</strong>`
                    :""
                }
                ${localizacion&&localizacion.nombre
                    ?`<div>📍 ${escaparHTML(localizacion.nombre)}</div>`
                    :""
                }
                ${estrato&&estrato.nombre
                    ?`<div>↕️ ${escaparHTML(estrato.nombre)}</div>`
                    :""
                }
                ${comportamiento&&comportamiento.nombre
                    ?`<div>🧭 ${escaparHTML(comportamiento.nombre)}</div>`
                    :""
                }
            </div>
        `;
...
/* ==========================================================
   MOSTRAR COMBATIENTES PREPARADOS
   ========================================================== */
...
function mostrarPreparadosArena(c1,c2,b1,b2,escenario){
    ...
    document.getElementById("combatientesPreparados").innerHTML=`
        <div>
            ...
            ❤️ HP inicial: ${numeroArena(c1.hp_max)}<br>
        ...
        <div>
            ...
            ❤️ HP inicial: ${numeroArena(c2.hp_max)}<br>
    ...
    if(panel&&escenario){
        ...
        const habitats=Array.isArray(escenario.habitats)
            ?escenario.habitats.map(function(c){

                const habitat=
                    window.PALARENA_DATOS.obtenerHabitat(c);
        ...
        const modo=
            window.PALARENA_DATOS.obtenerModo(
                escenario.modo
            );
...
async function nuevoCombateArena(){
    ...
    try{
        ...
        document.getElementById("codigo1").textContent=
            datos1.codigo;

        document.getElementById("codigo2").textContent=
            datos2.codigo;
...
function ejecutarCombateAutomaticoArena(){
    ...
    try{
        ...
        document.getElementById(
            ...
            🏆 Ganador:
            <strong>
                ${escaparHTML(resultado.ganador)}
            </strong>
        `;

        const hp1=
            numeroArena(
                resultado.combatiente1.hp
            );

        const hpMax1=
            numeroArena(
                resultado.combatiente1.hp_max
            );

        const hp2=
            numeroArena(
                resultado.combatiente2.hp
            );

        const hpMax2=
            numeroArena(
                resultado.combatiente2.hp_max
            );
...
/* ==========================================================
   RECIBIR SELECCIÓN DESDE EL VISOR
   ========================================================== */
...
/* ==========================================================
   COMPROBAR SELECCIÓN DESDE EL VISOR
   ========================================================== */
